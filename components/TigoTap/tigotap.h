...
namespace esphome {
    namespace tigo_tap {
        ...
        static const char barcode_crc_map[] = "GHJKLMNPRSTVWXYZ";
        ...
        class Sensor : public sensor::Sensor, public PollingComponent {
            public:
                void setup() override;
                void loop() override;
                ...
                void setBarcode(std::string barcode) { this->_barcode = barcode; };
                std::string getBarcode() { return this->_barcode; };
                ...
                void setPvNodeId(uint16_t pvNodeId) { this->_pvNodeId = pvNodeId; };
                uint16_t getPvNodeId()  { return this->_pvNodeId; };

                void setSensorTimeout(uint16_t seconds) { this->_timeout = seconds*1000; };
                uint16_t getSensorTimeout() { return this->_timeout/1000; };
        ...
        class FrameBuffer {
            public:
                ...
                int remove(uint16_t numOfBytes);
                uint16_t endWord();
                int length();
                void reset();
            ...
            private:
                uint8_t _frameBuffer[MAX_FRAME_SIZE];
                uint16_t _index;
        };



        class TigoTap : public uart::UARTDevice, public PollingComponent {
            public:
                TigoTap();
                void setup() override;
                void loop() override;
                ...
                void register_sensor(Sensor *obj, std::string barcode);
            ...
            private:
                ...
                uint8_t calcFrameHeaderLength(FrameBuffer* frame);

                bool processPowerPacket(FrameBuffer* frame, uint16_t position);
                bool processTopologyReport(FrameBuffer* frame, uint16_t position);
                bool processNodeTable(FrameBuffer* frame, uint16_t position);
                
                void generateBarcodeFromAddress(std::string* barcode, uint8_t* address);
                void updateBarcodeTable(std::string* barcode, uint16_t pvID);
        };
